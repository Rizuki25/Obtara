# Template Laporan Dry Run Internal

Status: Draft / Selesai / Perlu dry run ulang  
Tanggal: __________  
Participant ID: `DRY-__`  
Moderator: __________  
Peran simulasi: Pengguna obat / Caregiver / Lainnya  
Keterlibatan peserta dalam implementasi: Tidak / Ya, jelaskan __________  
Perangkat/browser: __________  
Versi/build: __________

> Dry run menilai kesiapan instrumen dan operasi sesi. Jangan gabungkan hasilnya dengan
> skor, waktu, atau kutipan peserta nyata.

## Kelengkapan artefak

- [ ] Informasi dan consent dibaca serta keputusan dicatat.
- [ ] Lembar observasi peserta dibuat.
- [ ] Kuesioner setelah tes diisi.
- [ ] Waktu mulai/selesai dan seluruh reset dicatat.
- [ ] Tidak ada data kesehatan nyata yang disebut atau disimpan.

## Ringkasan pelaksanaan

| Atribut               | Hasil      |
| --------------------- | ---------- |
| Waktu mulai / selesai |            |
| Durasi total          |            |
| Tugas selesai         | ____ / 12  |
| Jumlah reset          |            |
| Gangguan teknis       |            |
| Safety stop           | Ya / Tidak |

## Evaluasi integrasi

| Pertanyaan                                                     | Ya / Tidak | Bukti atau catatan |
| -------------------------------------------------------------- | ---------- | ------------------ |
| Instruksi moderator dapat dibacakan tanpa penjelasan tambahan? |            |                    |
| Tugas tidak membocorkan lokasi kontrol?                        |            |                    |
| Urutan tugas mempertahankan state yang dibutuhkan?             |            |                    |
| Reload mengembalikan baseline dengan cepat?                    |            |                    |
| Disclosure data simulasi dipahami?                             |            |                    |
| Popup Tunda, Lewati, dan Tidak Yakin dapat diuji?              |            |                    |
| Skor 0/1/2 mudah diterapkan?                                   |            |                    |
| Lembar observasi dapat diisi sambil memoderasi?                |            |                    |
| Durasi total berada pada 30–40 menit?                          |            |                    |
| Safety guardrail dapat dijalankan tanpa memberi arahan medis?  |            |                    |

## Log isu instrumen

| ID        | Instrumen/tugas | Masalah dan bukti | Dampak | Perubahan | Status |
| --------- | --------------- | ----------------- | ------ | --------- | ------ |
| DRY-F-001 |                 |                   |        |           |        |

## Keputusan

- Siap merekrut `U-*` dan `C-*`: Ya / Tidak
- Alasan: ________________________________________________________
- Perubahan wajib sebelum rekrutmen: ______________________________
- Perlu dry run ulang: Ya / Tidak
- Kriteria lulus dry run ulang: ___________________________________

## Batas bukti

Catat bagian yang tidak dapat dinilai pada dry run ini, misalnya pemahaman spontan,
think-aloud, waktu manusia, aksesibilitas dengan teknologi bantu, atau kenyamanan moderator.
